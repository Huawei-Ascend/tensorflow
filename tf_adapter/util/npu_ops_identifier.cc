...
NpuOpsIdentifier::NpuOpsIdentifier(bool is_mix, json &ops_info) : is_mix_(is_mix), ops_info_(ops_info) {
  ...
  if (path_env != nullptr && strlen(path_env) < ADAPTER_ENV_MAX_LENTH) {
    ...
    LOG(INFO) << "environment variable ASCEND_OPP_PATH is not set, use default value["
              << opsPath << "]";
  }
...
bool NpuOpsIdentifier::IsNpuSupported(const char *op) {
  return NpuOpsIdentifier::IsNpuSupported(std::string(op));
}
bool NpuOpsIdentifier::IsNpuSupported(const std::string &op) {
  bool declared = ops_info_[op].is_object();
  if (!declared) {
    return false;
  }
  if (is_mix_ && ops_info_[op][kGray].is_boolean()) {
    return !ops_info_[op][kGray];
  }
  return true;
}
...
bool NpuOpsIdentifier::IsPerformanceSensitive(const std::string &op) {
  auto iter = ops_info_.find(op);
  if (iter != ops_info_.end()) {
