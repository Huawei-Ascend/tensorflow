...
Status OMSubGraphToFunctionDef(const Graph &graph, const string &name,
                               FunctionDef *fdef) {
  ...
  for (Node const *node : graph.op_nodes()) {
    ...
    *nodeDef = node->def();
    ...
    // Add regular inputs
    for (std::vector<const Edge *>::size_type i = 0; i < inEdges.size(); ++i) {
      const Edge *edge = inEdges[i];
      REQUIRES_NOT_NULL(edge);
      REQUIRES_NOT_NULL(edge->src());
      nodeDef->add_input(
          strings::StrCat(edge->src()->name(), ":", edge->src_output()));
    ...
    TF_RETURN_IF_ERROR(
        NameRangesForNode(*node, node->op_def(), nullptr, &outputRanges));
    for (const auto &output : outputRanges) {
      for (int i = output.second.first; i < output.second.second; ++i) {
        const string tensorName = strings::StrCat(
            nodeDef->name(), ":", output.first, ":", i - output.second.first);
      ...
      if (str_util::StartsWith(nodeDef->input(i), "^")) {
        ...
        if (inputCtrlName.empty()) {
          return errors::InvalidArgument(
              "Could not remap control input ", i, ", '", nodeDef->input(i),
              "', of node '", nodeDef->name(), "' in function ", name);
        }
        *nodeDef->mutable_input(i) = strings::StrCat("^", inputCtrlName);
        ...
        if (iter == tensorRenaming.end()) {
          return errors::InvalidArgument(
              "Could not remap input ", i, ", '", nodeDef->input(i),
              "', of node '", nodeDef->name(), "' in function ", name);
        }
...
bool IsWhiteListSupport(const string &op_name, bool mix_compile_mode) {
  ...
  bool ans = (identifier->IsNpuSupported(op_name)) &&
      !EndsWith(op_name, suffix_op) &&
      !EndsWith(op_name, suffix_op_v2) &&
      !(op_name == "Const") &&
      !(op_name == "_Arg") &&
      !(op_name == "_Retval") &&
      !(op_name == "StringJoin");
...
bool IsNpuSupportingFunc(string func_name, FunctionLibraryDefinition *func_lib, int depth) {
  ...
  for (NodeDef node_def : func_def->node_def()) {
    ...
    } else if (!IsWhiteListSupport(node_def.op(), compile_mode) || IsWithoutNpuScope(node_def)) {
      return false;
    }
    for (auto item : node_def.attr()) {
...
bool IsNpuSupportingFunc(Node *node, FunctionLibraryDefinition *func_lib,
                         int depth) {
...
Status FindNpuSupportCandidates(const Graph &graph, OrderedNodeSet *candidates,
  ...
  LOG(INFO) << "FindNpuSupportCandidates enableDP:" << enableDP
            << ", mix_compile_mode: " << compile_mode
            << ", hasMakeIteratorOp:" << hasMakeIteratorOp
            << ", hasIteratorOp:" << hasIteratorOp;
  ...
  for (Node *node : sortedNodes) {
    ...
    if (enableDP &&
        (node->type_string() == "Iterator" ||
            node->type_string() == "IteratorV2" ||
      ...
      if (node->type_string() == "Const") {
        ...
        for (auto edge : node->in_edges()) {
          ...
          if (edge->IsControlEdge() && edge->src()->name() != "_SOURCE" &&
              IsWhiteListSupport(edge->src()->type_string(), mix_compile_mode) &&
      ...
      if (IsWhiteListSupport(node->type_string(), mix_compile_mode) &&
                             !IsWithoutNpuScope(node)) {
        candidates->insert(node);
      } else {
        outSet.insert(node);
      }
    ...
    for (auto edge : node->out_edges()) {
      ...
      if (!edge->IsControlEdge()) {
        ...
        if (IsRefType(dtypeDst)
            && candidates->count(edge->dst()) > 0) {
          candidates->erase(edge->dst());
          outSet.insert(edge->dst());
          LOG(INFO) << "Remove node : " << edge->dst()->name() <<
                    " from candidates, because of node : " <<
                    node->name() << " REF input.";
          continue;
        }
  ...
  LOG(INFO) << "TFadapter find Npu support candidates cost: ["
...
bool GetNodeFuncs(const FunctionLibraryDefinition *flib_def,
                  Node *node, std::vector<string> &nodeFuncs) {
  nodeFuncs.clear();
...
void MergeClusters(Edge *edge,
  ...
  for (auto start_node_name : cluster_dst->start_nodes_name) {
...
Status MergeSubgraphsInNewWay(std::vector<std::pair<string, int>> &sortedCluster,
  ...
  int64 endTime = InferShapeUtil::GetCurrentTimestap();
  LOG(INFO) << "TFadapter merge clusters cost: ["
            << ((endTime - startTime) / kMicrosToMillis) << " ms]";
  return Status::OK();
}

Status MergeSubgraphs(std::vector<std::pair<string, int>> &sortedCluster,
                      OrderedNodeSet &npuSupportCandidates,
                      std::map<string, std::set<string>> &clusterToMerge) {
  int64 startTime = InferShapeUtil::GetCurrentTimestap();
  std::set<string> mergedClusters;
  ...
  for (uint32_t i = 1; i < sortedCluster.size(); i++) {
    ...
    if (clusterToMerge[dstSubgraph].count(name) > 0 &&
        clusterToMerge[name].count(dstSubgraph) > 0) {
      canMerge = true;
      for (const string &mergedName : mergedClusters) {
        // Mutual exclusion between merged groups
        if (clusterToMerge[mergedName].count(name) == 0 ||
            clusterToMerge[name].count(mergedName) == 0) {
          canMerge = false;
          break;
        }
      }
...
Status MarkForPartition(std::unique_ptr<Graph>* graphIn,
  ...
  OrderedNodeSet npuSupportCandidates;
  TF_RETURN_IF_ERROR(FindNpuSupportCandidates(*graph, &npuSupportCandidates,
                                              func_lib, enableDP, mix_compile_mode));
  ...
  for (auto node : graph->nodes()) {
    ...
    if (node->attrs().Find("_StartNodeName")) {
      ...
      for (auto startNodeName : startNodeVec) {
  ...
  do {
    ...
    for (auto edge : graph->edges()) {
      ...
      if (cycles.HasEdge(src_index, dst_index) &&
          cycles.ContractEdge(src_index, dst_index)) {
          if (job != "localhost") {
            bool find_same_start = false;
            auto cluster_src = cluster_map[src];
            auto cluster_dst = cluster_map[dst];
            for (auto src_start_name : cluster_src->start_nodes_name) {
              for (auto dst_start_name : cluster_dst->start_nodes_name) {
                if (src_start_name == dst_start_name) {
                  find_same_start = true;
                  LOG(INFO) << "node : " << src->name() << " and node : "<<
                            dst->name() << " has same start node : " << src_start_name;
                  break;
                }
              }
              if (find_same_start) {
  ...
  for (auto item : cluster_map) {
    ...
    if (hasSupportNode && hasNonSupportNode) {
      LOG(INFO) << "Cluster " << cluster->index
                << " has both Candidate and non-Candidate nodes";
    ...
    string name = strings::StrCat(string(op_prefix), std::to_string(graph_num),
                                  string("_"), std::to_string(clusterSequenceNum++));
        ...
        if (!cycles.IsReachableNonConst(src, dst) && !cycles.IsReachableNonConst(dst, src)) {
          if (mix_compile_mode) {
            ...
            if (!canReach && job != "localhost") {
              ...
              for (auto cluster_item : cluster_map) {
                if (cluster_src == nullptr && src == cluster_item.second.get()->index) {
                  cluster_src = cluster_item.second.get();
                }
                if (cluster_dst == nullptr && dst == cluster_item.second.get()->index) {
  ...
  struct ClusterCompare {
    bool operator()(std::pair<string, int> a, std::pair<string, int> b) const {
      return a.second > b.second;
    }
  };

  std::vector<std::pair<string, int>> sortedCluster;
  for (auto cluster : clusterInfo) {
    sortedCluster.push_back(cluster.second);
  }
  ...
  if (static_cast<int>(sortedCluster.size()) != clusterNum) {
    return errors::Internal("Sorted cluster size should be equal to origin subgraph num. ",
                            "Sorted cluster size is ", sortedCluster.size(),
                            ", origin subgraph num is ", clusterNum);
  }
  ...
  int minGroupSize = (((minGroupSizeTemp > 0) && (minGroupSizeTemp < MAX_GROUP_SIZE))
                      ? (minGroupSizeTemp) : (1)); // default threshold is 10.
  LOG(INFO) << "All nodes in graph: " << graph->num_nodes() << ", max nodes count: "
            << sortedCluster[0].second << " in subgraph: " << sortedCluster[0].first
...
Node *AddIdentityNode(Graph *graph, const Edge *edge, string srcName, int srcIndex,
  ...
  NodeDefBuilder builder(
      strings::StrCat(edge->src()->name(), "_dummyIdentity"), "Identity");
 ...
 public:
  OMSplitter(string groupAttribute, Graph const *graphIn, std::map<std::string, std::string> npu_optimizer_options,
      ...
      : groupAttribute_(std::move(groupAttribute)),
        graphIn_(graphIn), npu_optimizer_options_(npu_optimizer_options), pass_options_(pass_options){}
  ...
  Status BuildFunctionDefs(FunctionLibraryDefinition *library,
                           string graph_format);
 ...
 private:
  ...
  class Subgraph {
   public:
    ...
    Subgraph()
      : GEOpNodeInputs_(nullptr), GEOpNodeOutputs_(nullptr){}
    ...
    Status BuildFunctionDef(const string &nameIn,
  ...
  Status CopyNodesToOutputGraph(Graph *graphOut,
  ...
  Status FindOutputImageOfEdgeSrc(
      const string &srcSubgraphId, const string &dstSubgraphId,
      const std::unordered_map<const Node *, Node *> &nodeImages,
  ...
  int FindOutputSlotOfEdgeSrc(const string &srcSubgraphId,
                              const string &dstSubgraphId,
                              const Edge *edge);
  ...
  // it is a slot on a node in the output graph.
  int FindOutputSlotOfEdgeDst(const string &srcSubgraphId, const string &dstSubgraphId,
                              const Edge *edge);
...
Node *OMSplitter::Subgraph::MakeNodeImage(const Graph *graphIn, Node *node) {
  ...
  if (device_.empty()) {
    if (job != "localhost" && job != "ps" && job != "defalut") {
...
Status OMSplitter::Subgraph::RecordArg(
    const Edge *edge, const std::unordered_map<const Node *, Node *> &nodeImages,
    std::vector<std::pair<const Node *, Node *>> *srcArgPairs) {
  ...
  std::tie(iter, inserted) =
      argsBySrc_.emplace(NodeSlot(srcNode, srcSlot), argsBySrc_.size());
  int argIndex = iter->second;
  if (inserted) {
    NodeDef argNodeDef;
    NodeDefBuilder builder(
        strings::StrCat(srcNode->name(), "_", srcSlot, "_arg"), ARG_OP);
...
Status OMSplitter::Subgraph::RecordResult(
    const Edge *edge,
    const std::unordered_map<const Node *, Node *> &nodeImages) {
  Node *srcNode = edge->src();
  ...
  bool inserted = false;
  std::tie(iter, inserted) =
      results_.emplace(NodeSlot(srcNode, srcSlot), results_.size());
  int retIndex = iter->second;
  if (inserted) {
    NodeDef retNodeDef;
    NodeDefBuilder builder(
        strings::StrCat(srcNode->name(), "_", srcSlot, "_retval"), RET_OP);
...
Status OMSplitter::Subgraph::BuildFunctionDef(
    const string &nameIn, FunctionLibraryDefinition *library, const string graph_format) {
  string name = nameIn;
  ...
  *function.mutable_attr() = GEOpNodeDef_.attr();
  ...
  for (auto option : npu_optimizer_options_) {
...
Status OMSplitter::Subgraph::AddGEOpNode(const std::unordered_map<const Node *, Node *> &nodeImages,
                                         Graph *graphOut) {
  Status s;
...
bool OMSplitter::Subgraph::isIsolatedSubgraph() {
  return false;
}
...
Status OMSplitter::Subgraph::SetOptions(std::map<std::string, std::string> npu_optimizer_options,
                                            std::map<std::string, std::string> pass_options)
{
  npu_optimizer_options_ = npu_optimizer_options;
  pass_options_ = pass_options;
  return Status::OK();
}
...
Status OMSplitter::CopySubgraphEdges(const std::unordered_map<const Node *, Node *> &nodeImages,
  ...
  for (const Edge *edge : graphIn_->edges()) {
    ...
    if (IsInSubgraph(srcSubgraphId) && IsInSubgraph(dstSubgraphId) &&
        srcSubgraphId == dstSubgraphId) {
    ...
    if (IsInSubgraph(srcSubgraphId)) {
      ...
      if (!edge->IsControlEdge()) {
        TF_RETURN_IF_ERROR(srcSubgraph.RecordResult(edge, nodeImages));
      }
    }
...
Status OMSplitter::BuildFunctionDefs(FunctionLibraryDefinition *library,
                                     string graph_format) {
...
Status OMSplitter::CopyNodesToOutputGraph(Graph *graphOut,
                                          std::unordered_map<const Node *, Node *> *nodeImages) {
  for (Node *node : graphIn_->op_nodes()) {
    string subgraphId;
    TF_RETURN_IF_ERROR(
        GetSubgraphIdAttr(node, &subgraphId));
...
Status OMSplitter::AddGEOpNodes(
    const std::unordered_map<const Node *, Node *> &nodeImages, Graph *graphOut) {
  for (auto &subgraphEntry : subgraphs_) {
    TF_RETURN_IF_ERROR(subgraphEntry.second.AddGEOpNode(
        nodeImages, graphOut));
  }
  return Status::OK();
}

Status OMSplitter::FindOutputImageOfEdgeSrc(
    const string &srcSubgraphId, const string &dstSubgraphId,
    const std::unordered_map<const Node *, Node *> &nodeImages,
    const Node *originalSrcNode, Node **srcImage) {
  if (IsInSubgraph(srcSubgraphId)) {
...
int OMSplitter::FindOutputSlotOfEdgeSrc(
    const string &srcSubgraphId, const string &dstSubgraphId,
    const Edge *edge) {
  if (IsInSubgraph(srcSubgraphId)) {
...
Status OMSplitter::FindOutputImageOfEdgeDst(
    const string &srcSubgraphId, const string &dstSubgraphId,
    const std::unordered_map<const Node *, Node *> &nodeImages,
    const Node *originalDstNode, Node **dstImage) {
  if (IsInSubgraph(dstSubgraphId)) {
...
int OMSplitter::FindOutputSlotOfEdgeDst(
    const string &srcSubgraphId, const string &dstSubgraphId,
    const Edge *edge) {
  if (IsInSubgraph(dstSubgraphId)) {
...
Status OMSplitter::CopyEdgeToOutputGraph(const Edge *edge,
     const string &srcSubgraphId,
     const string &dstSubgraphId,
     const std::unordered_map<const Node *, Node *> &nodeImages,
     Graph *graphOut,
  ...
  TF_RETURN_IF_ERROR(FindOutputImageOfEdgeSrc(
      srcSubgraphId, dstSubgraphId, nodeImages, edge->src(), &srcImage));
  Node *dstImage = NULL;
  ...
  if (edge->IsControlEdge()) {
    ...
    // determined above.
    if (edges_added->emplace(NodeSlot(srcImage, -1), NodeSlot(dstImage, -1))
        .second) {
  ...
  int srcOutput =
      FindOutputSlotOfEdgeSrc(srcSubgraphId, dstSubgraphId, edge);
  int dstInput =
      FindOutputSlotOfEdgeDst(srcSubgraphId, dstSubgraphId, edge);
  // Add the edge, if we have not already added it.
  if (edges_added
      ->emplace(NodeSlot(srcImage, srcOutput),
                NodeSlot(dstImage, dstInput)).second) {
    if (std::find(refIn_.begin(), refIn_.end(), edge) != refIn_.end()) {
      Status status;
      Node *identityNode = AddIdentityNode(graphOut, edge, srcImage->name(), srcOutput,
                                           srcImage->assigned_device_name(), &status);
...
Status OMSplitter::AddEdgesToOutputGraph(const std::unordered_map<const Node *, Node *> &nodeImages,
                                         Graph *graphOut) {
  ...
  for (const Edge *edge : graphIn_->edges()) {
    ...
    TF_RETURN_IF_ERROR(CopyEdgeToOutputGraph(
        edge, srcSubgraphId, dstSubgraphId, nodeImages, graphOut, &edges_added));
  }
...
Status OMPartitionSubgraphsInFunctions(
    string groupAttribute, std::unique_ptr<Graph>* graph, string graph_format,
    FunctionLibraryDefinition* flib_def, std::map<std::string, std::string> npu_optimizer_options,
    std::map<std::string, std::string> pass_options) {
  Graph *graphIn = graph->get();
  FunctionLibraryDefinition *const library = flib_def;

  OMSplitter omsplitter(std::move(groupAttribute), graphIn, npu_optimizer_options, pass_options);
  ...
  TF_RETURN_IF_ERROR(
      omsplitter.BuildOutputGraph(out.get()));
  *graph = std::move(out);
                                              ...
                                              const OptimizationPassRegistry::Grouping pass_group_value)
{
  ...
  for (Node *n : graph->get()->nodes()) {
    ...
    if (n->attrs().Find("_NoNeedOptimize")) {
      LOG(INFO) << "Found mark of noneed optimize on node ["
                << n->name() << "], skip OMPartitionSubgraphsPass.";
      return Status::OK();
    }
  }
  ...
  if (job == "localhost" &&
      pass_group_value != OptimizationPassRegistry::POST_REWRITE_FOR_EXEC) {
    return Status::OK();
  }
  if (job != "localhost" &&
      pass_group_value != OptimizationPassRegistry::POST_PARTITIONING) {
    return Status::OK();
  }
  ...
  for (Node *node : graphIn->op_nodes()) {
    ...
    if (node->type_string() == "_UnaryOpsComposition") {
      ...
      for (int i = 0; i < node_list.s_size(); i++) {
        string node_name = node_list.s(i);
  ...
  int subgraphNum = 0;
  TF_RETURN_IF_ERROR(OMSplitter::MarkForPartition(graph, subgraphNum, mix_compile_mode, graph_num, func_lib, pass_options));
  LOG(INFO) << "OMPartion subgraph_" << std::to_string(graph_num)
            << " markForPartition success.";
  if (subgraphNum < 1) {
    ...
    for (auto varEdge : varEdges) {
      ...
      if (s.code() == error::Code::NOT_FOUND) {
        if (!IsRefType(dtypeDst)) {
          ...
          return errors::InvalidArgument("Ref Tensors (e.g., Variables) output: ",
                                         varEdge->dst()->name(), " is not in white list");
        }
      } else {
        TF_RETURN_IF_ERROR(s);
      }
  ...
  TF_RETURN_IF_ERROR(OMSplitter::OMPartitionSubgraphsInFunctions(
      OMSplitter::PARTITION_SUB_GRAPH_ATTR, graph, graph_format_value, func_lib,
      all_options, pass_options));
  LOG(INFO) << "OMPartion subgraph_" << std::to_string(graph_num)
            << " SubgraphsInFunctions success.";
  ...
  int64 endTime = InferShapeUtil::GetCurrentTimestap();
  LOG(INFO) << "OMPartion subgraph_" << std::to_string(graph_num) << " success. ["
            << ((endTime - startTime) / kMicrosToMillis) << " ms]";
  return Status::OK();
}

REGISTER_OPTIMIZATION(OptimizationPassRegistry::POST_REWRITE_FOR_EXEC, 2,
                      OMPartitionSubgraphsPass);
REGISTER_OPTIMIZATION(OptimizationPassRegistry::POST_PARTITIONING, 101,
                      OMPartitionSubgraphsPass);
} // namespace tensorflow
