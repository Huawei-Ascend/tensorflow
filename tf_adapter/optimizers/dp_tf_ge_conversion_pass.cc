...
const static std::vector<std::string> GE_OPS_WHITELIST = {
    "MapDataset",         "ParallelMapDataset", "BatchDataset",
    "MapAndBatchDataset", "DeviceQueueDataset",
    "BatchDatasetV2",     "MapAndBatchDatasetV2",
    "ModelDataset", "OptimizeDataset"};

// Customize dataset list
const static std::vector<std::string> CUSTOMIZE_DATASET_LIST = {
    "BatchDataset", "BatchDatasetV2", "MapAndBatchDataset",
    "MapAndBatchDatasetV2", "ParallelMapDataset",
    "MakeIterator"};
// Skip dataset list
const static std::vector<std::string> SKIP_DATASET_LIST = {
    "ModelDataset", "OptimizeDataset"
};

// GE fun black list
const static std::vector<std::string> GE_FUN_BLACKLIST = {
    "PyFunc",
    "SaveV2",
    "RestoreV2",
    "MergeV2Checkpoints",
...
class DpTfToGEConversionPassImpl {
 public:
  explicit DpTfToGEConversionPassImpl()
      : graph_run_num_(0),
        graph_(nullptr),
        flib_def_(nullptr) {};
 ...
 private:
  ...
  bool RunPass(std::unique_ptr<Graph> *g, FunctionLibraryDefinition *flib, std::map<std::string, std::string> all_options);
  ...
  inline Status InsertChannelQueue(Node *topo_end, std::string &host_queue_name,
                                   std::string &device_queue_name) const;
  bool GetNodeFuncs(const FunctionLibraryDefinition *flib_def, Node *node,
                    std::vector<string> &node_funcs);
  bool RemoveIsolatedNode(Graph *g, std::unordered_set<Node *> visited);
  Status RemoveNotSupportDataset(Graph *g, std::string start, std::string end) const;
...
inline bool DpTfToGEConversionPassImpl::IsSkipDataset(const Node* n) const {
  return std::find(SKIP_DATASET_LIST.begin(), SKIP_DATASET_LIST.end(),
                  n->type_string()) != SKIP_DATASET_LIST.end();
}
...
inline std::string DpTfToGEConversionPassImpl::GetEdgeName(const Edge* e) const {
  ...
  return strings::StrCat("Edge_from_", e->src()->name(), "_out",
                        e->src_output(), "_To_", e->dst()->name(), "_in",
                        e->dst_input());
}

inline std::string DpTfToGEConversionPassImpl::GetRandomName(
...
std::string DpTfToGEConversionPassImpl::GetRandomName() const {
    ...
    for (size_t i = 0; i < RANDOM_LEN; i++) {
      ...
      x += ASCII_BASE[setIdx] + asciiIdx;
    }
    return x;
}

bool DpTfToGEConversionPassImpl::EndsWith(const std::string& str,
...
bool DpTfToGEConversionPassImpl::CheckNode(const std::string& op) const {
  ...
  if (EndsWith(op, suffix_op) || EndsWith(op, suffix_op_v2)) {
    if (std::find(GE_OPS_WHITELIST.begin(), GE_OPS_WHITELIST.end(), op) !=
        GE_OPS_WHITELIST.end()) {
      return true;
    } else {
      return false;
    }
  } else {
    if (std::find(GE_FUN_BLACKLIST.begin(), GE_FUN_BLACKLIST.end(), op) ==
        GE_FUN_BLACKLIST.end()) {
      return true;
    } else {
      return false;
    }
...
bool DpTfToGEConversionPassImpl::IsDeviceSupportedOp(const NodeDef& n) const {
  ...
  if (OpRegistry::Global()->LookUp(n.op(), &op_reg_data).ok()) {
      ...
      for (auto& attr : n.attr()) {
        if (attr.second.has_func()) {
          if (!IsDeviceSupportedFunc(attr.second.func().name())) {
            LOG(INFO) << "Node [" << n.name() << "] function ["
                      << attr.second.func().name() << "] not supported by GE";
            return false;
          }
        }
      }
    ...
    if (!IsDeviceSupportedFunc(n.op())) {
      LOG(INFO) << "Node [" << n.name() << "] op [" << n.op()
                << "] is not a supported function by GE";
      return false;
    }
  }
  return true;
...
bool DpTfToGEConversionPassImpl::IsDeviceSupportedFunc(
  ...
  for (const NodeDef& node : fdef->node_def()) {
    if (!IsDeviceSupportedOp(node)) {
      LOG(INFO) << "Function [" << fn << "] node [" << node.name()
                << "] not supported by GE";
      return false;
    }
  }
...
Status DpTfToGEConversionPassImpl::GetSplitEdges(
    const Node* n,
    std::vector<const Edge*>& split_edges,
...
Status DpTfToGEConversionPassImpl::InsertChannelQueue(
    Node* topo_end,
  ...
  for (const Edge* e : split_edges_.at(topo_end)) {
    ...
    std::string queue_name =
        strings::StrCat("Queue_", GetEdgeName(e), "_", GetRandomName());
    ...
    TF_CHECK_OK(NodeBuilder(host_queue_name, "HostQueueDataset")
                    ...
                    .Attr("output_types", type_status ? m_src["output_types"]
                                                      : m_src["Toutput_types"])
    ...
    REQUIRES_NOT_NULL(
        graph_->AddEdge(queue_node_device, 0, e->dst(), e->dst_input()));
  }
  return Status::OK();
}

Status DpTfToGEConversionPassImpl::RemoveNotSupportDataset(
    Graph* g,std::string device_queue_dataset, std::string make_iterator) const {
  ...
  while (!IsMakeIteratorNode(node)) {
    ...
    if (node->num_outputs() != 1) {
      LOG(ERROR) << "Invalid node " << node->name()
                << ", op is" << node->type_string();
      return errors::InvalidArgument(
          "RemoveSplitDataset: find invalid node.");
    }
...
bool DpTfToGEConversionPassImpl::GetNodeFuncs(
    const FunctionLibraryDefinition* flib_def, Node* node,
    std::vector<string>& node_funcs) {
  node_funcs.clear();
...
bool DpTfToGEConversionPassImpl::RunPass(std::unique_ptr<Graph> *g, FunctionLibraryDefinition *flib,
  ...
  if (nullptr != need_print && strcmp("1", need_print) == 0) {
    ...
    string
        pmodel_path = pre_model_path + std::to_string(graph_run_num_) + ".pbtxt";
  ...
  for (Node* topo_end : topo_ends) {
    ...
    LOG(INFO) << "Start to find split edges, topo_end node is : "
              << topo_end->name() << ", op is " << topo_end->type_string();
    ...
    for (auto node : graph_ge->nodes()) {
      ...
      if (std::find(CUSTOMIZE_DATASET_LIST.begin(), CUSTOMIZE_DATASET_LIST.end(),
                    node->type_string()) != CUSTOMIZE_DATASET_LIST.end()) {
  ...
  }
  {
    ...
    *f_attr.mutable_attr() = n->attr();
    TF_CHECK_OK(
            NodeDefBuilder(GetRandomName("GeOp"), "GeOp")
                    .Input(EMPTY_DEF_INPUT)  // No partition dp_init graph on GE
                    .Device(DEFAULT_DEVICE)
  ...
  flib->AddLibrary(fdeflib);
    ...
    std::vector<const Edge*> topo_end_input_edges(topo_end->in_edges().begin(),
                                                  topo_end->in_edges().end());
    ...
    std::unordered_set<Node*> isolated_nodes;
    {
      ...
      for (Node* n : graph_->op_nodes()) {
        ...
        if (n->type_string().compare("HostQueueDataset") == 0 &&
              n->name() == host_queue_name) {
          // 0: Host queue always generate one dataset
          LOG(INFO) << "inputs add node : name is " << n->name()
                    << ", op is " << n->type_string();
          inputs.push_back(NodeBuilder::NodeOut(n, 0));
        }
        if (n->type_string().compare("DeviceQueueDataset") == 0 &&
              n->name() == device_queue_name) {
          isolated_nodes.insert(n);
        }
      }

      TF_CHECK_OK(
          NodeBuilder(GetRandomName("DPGroupDataset"), "DPGroupDataset")
      ...
      TF_CHECK_OK(
          NodeBuilder(GetRandomName("GeopDataset"), "GEOPDataset")
              .Device(DEFAULT_DEVICE)
    ...
    for (const Edge* e : topo_end_input_edges) {
      if (!IsIteratorNode(e->src())) {
        CHECK_NOTNULL(
            graph_->AddEdge(dpgroup_dataset_node, 0, e->dst(), e->dst_input()));
        LOG(INFO) << "Remove_" << GetEdgeName(e);
        graph_->RemoveEdge(e);
      }
    }
    ...
    for (Node* n : (*g)->op_nodes()) {
      if (n->assigned_device_name().empty()) {
        ...
        LOG(INFO) << "Assigned node [" << n->name() << "] on device ["
  ...
  if (nullptr != need_print && strcmp("1", need_print) == 0) {
    ...
    string smodel_path =
        suffix_model_path + std::to_string(graph_run_num_) + ".pbtxt";
    TF_DO_CHECK_OK(WriteTextProto(Env::Default(), smodel_path, after_graphdef), ERROR);
  }
  return true;
}

bool DpTfToGEConversionPassImpl::RemoveIsolatedNode(
    Graph* g, std::unordered_set<Node*> visited) {
...
Status DpTfToGEConversionPassImpl::Run(
                                                ...
                                                const OptimizationPassRegistry::Grouping pass_group_value)
{
  ...
  auto process_graph = [&](std::unique_ptr<Graph> *g, FunctionLibraryDefinition *flib, std::map<std::string, std::string> all_options) {
    RunPass(g, flib, all_options);
  };

  // For any pre-partitioning phase, graph is stored in options.graph.
  process_graph(graph, func_lib, all_options);
  int64 endTime = InferShapeUtil::GetCurrentTimestap();
  LOG(INFO) << "DpTfToGEConversionPassImpl Run success. ["
...
REGISTER_OPTIMIZATION(OptimizationPassRegistry::POST_REWRITE_FOR_EXEC, 3,
                      DpTfToGEConversionPass);
REGISTER_OPTIMIZATION(OptimizationPassRegistry::POST_PARTITIONING, 102,
                      DpTfToGEConversionPass);
}  // namespace tensorflow
