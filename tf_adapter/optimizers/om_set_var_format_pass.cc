...
Status SetVarFormatPass::AssignApplyMomentumInNodesFormat(Node *node,
                                                          const string &var_format) {
  if (node == nullptr) {
    return Status::OK();
  }
  for (const Edge *in_edge : node->in_edges()) {
    ...
    bool is_momentum_op = (in_edge->dst_input() == 1) &&
        ((src_node->type_string() == KEY_VAR_HANDLE_OP_VALUE) ||
...
Status SetVarFormatPass::GetFormat(Node *node, string &format) {
  for (const Edge *out : node->out_edges()) {
    ...
    bool is_fz_node = (out->dst_input() == 1) &&
        ((dst_node->type_string() == KEY_CONV2D_OP_VALUE) ||
...
Status SetVarFormatPass::AssignFormatToVarOutNodes(Node *node) {
  ...
  for (const Edge *out : node->out_edges()) {
    ...
    Node *dst_node = out->dst();
    REQUIRES_NOT_NULL(dst_node);
    bool is_read_var_node = (dst_node->type_string() == KEY_IDENTITY_OP_VALUE) ||
        (dst_node->type_string() == KEY_READ_VARIABLE_OP_VALUE);
    if (is_read_var_node) {
...
Status SetVarFormatPass::Run(const GraphOptimizationPassOptions &options) {
  ...
  for (Node *node : graph_in->op_nodes()) {
    if ((node != nullptr) && ((node->type_string() == KEY_VAR_HANDLE_OP_VALUE) ||
        (node->type_string() == KEY_VARIABLE_V2_VALUE))) {
