  ...
  void MakeDataset(OpKernelContext* ctx, DatasetBase** output) override {
    ...
    *output = new (std::nothrow) Dataset(ctx, inputs, output_types_, output_shapes_);
    OP_REQUIRES(ctx, *output != nullptr,
      errors::Internal("Failed new dataset of DPGroupDatasetOp"));
  }
  ...
  class Dataset : public DatasetBase {
   public:
    explicit Dataset(OpKernelContext* ctx,
      ...
      output_types_.insert(output_types_.end(), output_types.begin(),
                           output_types.end());
      output_shapes_.insert(output_shapes_.end(), output_shapes.begin(),
                            output_shapes.end());
    }
