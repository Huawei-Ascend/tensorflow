...
class DecodeImageOp : public OpKernel {
  public:
    explicit DecodeImageOp(OpKernelConstruction* context) : OpKernel(context) {
      ...
      if (lib_path.empty()) {
        OP_REQUIRES_OK(context, errors::InvalidArgument("Find tf-plugin path faild!"));
      }
    ...
    void Compute(OpKernelContext* context) override {
      ...
      OP_REQUIRES(context, ret == 0,
                  errors::Internal("Decode jpeg failed, height is: ",
...
REGISTER_KERNEL_BUILDER(Name("DecodeAndCropAndResizeJpeg").Device(DEVICE_CPU),
                            DecodeImageOp);
}  // namespace tensorflow
