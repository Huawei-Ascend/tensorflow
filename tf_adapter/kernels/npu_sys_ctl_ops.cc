...
NPUInit::~NPUInit() {
  ...
  int64 unInitStartTime = GetCurrentTimestap();
  {
    mutex_lock lock{g_mu};
    if (g_npuInitNum > 0) {
      g_npuInitNum--;
    ...
    if (g_npuInitNum != 0) {
      int64 unInitEndTime = GetCurrentTimestap();
      LOG(INFO) << "[~NPUInit] NPU Shutdown success. ["
                << ((unInitEndTime - unInitStartTime) / kMicrosToMillis) << " ms]";
      return;
    }
  }
  if (!GePlugin::GetInstance()->IsGlobal()) {
