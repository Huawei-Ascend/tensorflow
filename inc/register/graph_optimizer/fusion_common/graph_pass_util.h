...
public:
   ...
   */
  static void SetOutputDescAttr(uint32_t originIndex, uint32_t fusionIndex,
    ...
    auto fusionNodeOutputDesc =
        fusionNode->GetOpDesc()->MutableOutputDesc(fusionIndex);
    ...
    auto originNodeOutputDesc =
        originNode->GetOpDesc()->MutableOutputDesc(originIndex);
    if (originNodeOutputDesc == nullptr) {
      return;
    }

    std::vector<std::string> originalNames;
    if (ge::AttrUtils::GetListStr(originNode->GetOpDesc(),
                                  ge::ATTR_NAME_DATA_DUMP_ORIGIN_OP_NAMES,
                                  originalNames) &&
        originalNames.size() > 0) {
      std::string originalName;
      if (ge::AttrUtils::GetStr(originNodeOutputDesc,
                                ge::ATTR_NAME_DATA_DUMP_ORIGIN_NAME,
                                originalName)) {
        (void)ge::AttrUtils::SetStr(fusionNodeOutputDesc,
                                    ge::ATTR_NAME_DATA_DUMP_ORIGIN_NAME,
                                    originalName);

        std::int64_t originOutputIndex = 0;
        if (ge::AttrUtils::GetInt(originNodeOutputDesc,
                                  ge::ATTR_NAME_DATA_DUMP_ORIGIN_OUTPUT_INDEX,
                                  originOutputIndex)) {
          (void)ge::AttrUtils::SetInt(
              fusionNodeOutputDesc, ge::ATTR_NAME_DATA_DUMP_ORIGIN_OUTPUT_INDEX,
              originOutputIndex);
        }

        ge::DataType originDataType =
            GetDataDumpOriginDataType(originNodeOutputDesc);
      ...
      (void)ge::AttrUtils::SetStr(fusionNodeOutputDesc,
                                  ge::ATTR_NAME_DATA_DUMP_ORIGIN_NAME,
                                  originNode->GetName());
      (void)ge::AttrUtils::SetInt(fusionNodeOutputDesc,
                                  ge::ATTR_NAME_DATA_DUMP_ORIGIN_OUTPUT_INDEX,
                                  originIndex);
      SetDataDumpOriginDataType(originNodeOutputDesc->GetOriginDataType(),
                                fusionNodeOutputDesc);
      SetDataDumpOriginFormat(originNodeOutputDesc->GetOriginFormat(),
                              fusionNodeOutputDesc);
    }
  }
  ...
  static ge::Format GetDataDumpOriginFormat(ge::GeTensorDescPtr tensorDesc) {
    ...
    if (!ge::AttrUtils::GetStr(tensorDesc,
                               ge::ATTR_NAME_DATA_DUMP_ORIGIN_FORMAT,
                               originFormatStr)) {
   ...
   * @param tensorDesc,usually is outputDesc
   */
  static void SetDataDumpOriginFormat(ge::Format originFormat,
                                      ge::GeTensorDescPtr tensorDesc) {
    ...
    (void)ge::AttrUtils::SetStr(
        tensorDesc, ge::ATTR_NAME_DATA_DUMP_ORIGIN_FORMAT, originFormatStr);
  }
   ...
   */
  static ge::DataType
  GetDataDumpOriginDataType(ge::GeTensorDescPtr tensorDesc) {
  ...
  static void AddNodeFromOpTypeMap(NodeMapInfoPtr &nodeMapInfo,
                                   ge::NodePtr &nodePtr) {
    ...
    if (iter != nodeTypeMap->end()) {
      ...
      nodeTypeMap->emplace(
          std::make_pair(realOpType, std::unordered_set<ge::NodePtr>{nodePtr}));
    }
  }

  static Status GetOpTypeMapToGraph(NodeMapInfoPtr &nodeMapInfo,
  ...
  static void RecordOriginalNames(std::vector<ge::NodePtr> originalNodes,
                                  ge::NodePtr node) {
    ...
    for (ge::NodePtr originalNode : originalNodes) {
      ...
      bool isHasAttr = ge::AttrUtils::GetListStr(
          originOpDescPtr, ge::ATTR_NAME_DATA_DUMP_ORIGIN_OP_NAMES, names_tmp);
      if (isHasAttr) {
    ...
    (void)ge::AttrUtils::SetListStr(
        nodeOpDescPtr, ge::ATTR_NAME_DATA_DUMP_ORIGIN_OP_NAMES, originalNames);
  }
};
