...
class ScopeAttrValue::ScopeAttrValueImpl {
 public:
  ...
  void SetFloatValue(float value) { float_value_ = value; }
  void SetStringValue(std::string value) { string_value_ = value; }
  void SetBoolValue(bool value) { bool_value_ = value; }
  int64_t GetIntValue() { return int_value_; }
  float GetFloatValue() { return float_value_; }
  std::string GetStrValue() { return string_value_; }
  bool GetBoolValue() { return bool_value_; }
...
class NodeOpTypeFeature::NodeOpTypeFeatureImpl : ScopeBaseFeature {
 public:
  NodeOpTypeFeatureImpl(std::string nodeType, int num, int step = 0) : node_type_(nodeType), num_(num), step_(step) {}
...
class ScopeFeature::ScopeFeatureImpl : ScopeBaseFeature {
 public:
  ...
  ~ScopeFeatureImpl() {}
  bool Match(const Scope *scope) override;
  bool SubScopesMatch(std::vector<Scope *> &scopes);
