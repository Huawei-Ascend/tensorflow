...
class ScopesResult::ScopesResultImpl {
 public:
  void SetScopes(std::vector<Scope *> &scopes) { scopes_ = scopes; }
  std::vector<Scope *> GetScopes() const { return scopes_; }
  void SetNodes(std::vector<ge::OperatorPtr> nodes) { nodes_ = nodes; }
  std::vector<ge::OperatorPtr> GetNodes() const { return nodes_; }
